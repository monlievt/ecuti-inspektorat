...
class KirimNotifikasiWhatsApp
{
    ...
    /**
     * Handle the event synchronously and safely.
     */
    public function handle(StatusCutiBerubah $event): void
    {
        try {
            ...
            if (!$pegawai || !$pengajuan->jenisCuti) {
                return;
            }

            $tglMulai = $pengajuan->tanggal_mulai ? $pengajuan->tanggal_mulai->format('d/m/Y') : '-';
            $tglSelesai = $pengajuan->tanggal_selesai ? $pengajuan->tanggal_selesai->format('d/m/Y') : '-';
            ...
            Log::warning('KirimNotifikasiWhatsApp failed gracefully: ' . $e->getMessage());
        }
    }
}
